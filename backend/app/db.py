...
def _ensure_schema() -> None:
    ...
    with engine.begin() as conn:
        ...
        safe_add(conn, "case_data", "zivilstand")
        safe_add(conn, "case_data", "aufenthaltsort_vor_eintritt")
        safe_add(conn, "case_data", "beschaeftigung_teilzeit", "INTEGER")
        safe_add(conn, "case_data", "beschaeftigung_vollzeit", "INTEGER")
        safe_add(conn, "case_data", "beschaeftigung_arbeitslos", "INTEGER")
        safe_add(conn, "case_data", "beschaeftigung_haushalt", "INTEGER")
        safe_add(conn, "case_data", "beschaeftigung_ausbildung", "INTEGER")
        safe_add(conn, "case_data", "beschaeftigung_reha", "INTEGER")
        safe_add(conn, "case_data", "beschaeftigung_iv", "INTEGER")
        safe_add(conn, "case_data", "schulbildung")
        # SpiGes Eintrittsmerkmale (v4)
        safe_add(conn, "case_data", "einweisende_instanz")
        safe_add(conn, "case_data", "behandlungsgrund")
        # SpiGes Austrittsmerkmale (v4)
        safe_add(conn, "case_data", "entscheid_austritt")
        safe_add(conn, "case_data", "aufenthalt_nach_austritt")
        safe_add(conn, "case_data", "behandlung_nach_austritt")
        safe_add(conn, "case_data", "behandlungsbereich")
        # FU (v4)
        safe_add(conn, "case_data", "fu_bei_eintritt")
        safe_add(conn, "case_data", "fu_typ")
        safe_add(conn, "case_data", "fu_datum")
        safe_add(conn, "case_data", "fu_gueltig_bis")
        safe_add(conn, "case_data", "fu_nummer", "INTEGER")
        safe_add(conn, "case_data", "fu_einweisende_instanz")
        # SpiGes Behandlungsdaten MP 3.4 (v5)
        safe_add(conn, "case_data", "behandlung_typ")
        safe_add(conn, "case_data", "neuroleptika", "INTEGER")
        safe_add(conn, "case_data", "depotneuroleptika", "INTEGER")
        safe_add(conn, "case_data", "antidepressiva", "INTEGER")
        safe_add(conn, "case_data", "tranquilizer", "INTEGER")
        safe_add(conn, "case_data", "hypnotika", "INTEGER")
        safe_add(conn, "case_data", "psychostimulanzien", "INTEGER")
        safe_add(conn, "case_data", "suchtaversionsmittel", "INTEGER")
        safe_add(conn, "case_data", "lithium", "INTEGER")
        safe_add(conn, "case_data", "antiepileptika", "INTEGER")
        safe_add(conn, "case_data", "andere_psychopharmaka", "INTEGER")
        safe_add(conn, "case_data", "keine_psychopharmaka", "INTEGER")
        # MB Minimaldaten (v5)
        safe_add(conn, "case_data", "eintrittsart")
        safe_add(conn, "case_data", "klasse")
        # Ack: Langlieger valid_until (v4)
        safe_add(conn, "ack", "ack_valid_until")
