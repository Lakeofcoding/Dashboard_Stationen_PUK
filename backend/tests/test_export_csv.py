...
class TestExportReports:
    ...
    def test_list_all_reports(self, client, admin_h):
        r = client.get("/api/export/reports?frequency=all&ctx=A1", headers=admin_h)
        assert r.status_code == 200
        data = r.json()
        assert isinstance(data, (list, dict))

    def test_list_daily_reports(self, client, admin_h):
        r = client.get("/api/export/reports?frequency=daily&ctx=A1", headers=admin_h)
        assert r.status_code == 200

    def test_list_weekly_reports(self, client, admin_h):
        r = client.get("/api/export/reports?frequency=weekly&ctx=A1", headers=admin_h)
        assert r.status_code == 200
...
class TestExportData:
    ...
    def test_export_data_with_report_id(self, client, admin_h):
        ...
        reports = client.get("/api/export/reports?ctx=A1", headers=admin_h).json()
        ...
        r = client.get(f"/api/export/data?report_id={rid}&ctx=A1", headers=admin_h)
        assert r.status_code == 200
...
class TestExportSummary:
    ...
    def test_summary_with_station_filter(self, client, admin_h):
        r = client.get("/api/export/summary?frequency=daily&station_id=A1&ctx=A1", headers=admin_h)
        assert r.status_code == 200
...
class TestCSVUpload:
    ...
    def test_upload_empty_csv_fails(self, client, auth):
        ...
        h_clean = {k: v for k, v in h.items() if k != "Content-Type"}
        r = client.post(
            "/api/admin/csv/upload?ctx=A1&station_id=A1",
            headers=h_clean,
