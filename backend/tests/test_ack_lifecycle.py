...
class TestRuleLevelAck:
    ...
    def test_ack_active_rule(self, client, auth, first_case_id):
        ...
        h = auth.admin()
        detail = client.get(f"/api/cases/{first_case_id}?ctx=A1", headers=h)
        ...
        r = client.post("/api/ack?ctx=A1", headers=h, json={
...
class TestCaseLevelAck:
    ...
    def test_case_ack_with_open_rules_fails(self, client, auth):
        ...
        h = auth.admin()
        cases = client.get("/api/cases?ctx=A1", headers=h).json()
...
class TestResetToday:
    ...
    def test_reset_increments_version(self, client, auth):
        ...
        h = auth.admin()
        before = client.get("/api/day_state?ctx=A1", headers=h).json()
        v_before = before["version"]

        r = client.post("/api/reset_today?ctx=A1", headers=h)
        assert r.status_code == 200

        after = client.get("/api/day_state?ctx=A1", headers=h).json()
...
class TestAckValidation:
    ...
    def test_invalid_scope(self, client, auth, first_case_id):
        r = client.post("/api/ack?ctx=A1", headers=auth.admin(), json={
            "case_id": first_case_id,
            "ack_scope": "INVALID",
            "scope_id": "*",
        })
